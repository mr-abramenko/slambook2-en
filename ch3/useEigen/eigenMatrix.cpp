...
// Core part of Eigen
#include <Eigen/Core>
// Algebraic operations on dense matrices
#include <Eigen/Dense>
...
int main(int argc, char** argv) {
  ...
  // At the same time, Eigen provides many built-in types through typedef, but
  // the bottom layer is still Eigen::Matrix. For example, Vector3d is
  // essentially Eigen::Matrix<double, 3, 1>, which is a three-dimensional
  // vector.
  Vector3d v_3d;
  // This is the same
  ...
  Matrix3d matrix_33 =
      Matrix3d::Zero();  // Initialized to zero
                         //  If you are not sure about the matrix size, you can
                         //  use a dynamically sized matrix
  Matrix<double, Dynamic, Dynamic> matrix_dynamic;
  // simpler
  MatrixXd matrix_x;
  // There are many more types of this type, so we won’t list them all.

  // The following is the operation of Eigen matrix
  ...
  // Multiply matrices and vectors (actually still matrices and matrices)
  v_3d << 3, 2, 1;
  vd_3d << 4, 5, 6;

  // But in Eigen you can't mix two different types of matrices, it's wrong like
  // this Matrix<double, 2, 1> result_wrong_type = matrix_23 * v_3d; should be
  ...
  // The size of N is defined in the previous macro and is generated by random
  // numbers
  // Direct inversion is naturally the most direct, but the inversion requires a
  // lot of operations

  Matrix<double, MATRIX_SIZE, MATRIX_SIZE> matrix_NN =
      MatrixXd::Random(MATRIX_SIZE, MATRIX_SIZE);
  matrix_NN =
      matrix_NN * matrix_NN.transpose();  // Guaranteed positive semi-definite
  ...
  // Usually matrix decomposition is used to find it, such as QR decomposition,
